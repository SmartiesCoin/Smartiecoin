# Smartiecoin Core version v19.0.0
...
first startup of Smartiecoin Core will run a migration process which can take anywhere
...
Smartiecoin Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by -wallet options on the command line or in
smartiecoin.conf or settings.json files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Smartiecoin Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the smartiecoin-wallet create command or the createwallet RPC.
...
- Testing or debugging of implementations of the Smartiecoin P2P network protocol
should be done by inspecting the log messages that are produced by a recent
version of Smartiecoin Core. Smartiecoin Core logs debug messages
...
- Passing an invalid `-rpcauth` argument now cause smartiecoind to fail to start.
...
This release introduces many updates from Bitcoin v0.18-v0.21 as well as numerous updates from Bitcoin v22 and more recent versions. Bitcoin changes that do not align with Smartiecoin’s product needs, such as SegWit and RBF, are excluded from our backporting. For additional detail on what’s included in Bitcoin, please refer to their release notes.

# v19.0.0 Change log

See detailed [set of changes](https://github.com/SmartiesCoin/Smartiecoin/compare/v18.2.2...dashpay:v19.0.0).
