# Smartiecoin Core version v20.0.0
...
Currently, 10% of block rewards are set aside for the Smartiecoin DAO treasury which
...
The asset lock process provides a mechanism for locking and unlocking Smartiecoin in a
credit pool. This credit pool mirrors the available credit supply in Smartiecoin
Platform. When a certain amount of Smartiecoin is locked, a proportional number of
credits are issued to the identity specified by the transaction issuer. The
identity then uses the credits to pay fees for operations done on Smartiecoin
Platform. Masternode identities receive the fees and can convert them back to
Smartiecoin by withdrawing them from Smartiecoin Platform.
...
### Removal of reject network messages from Smartiecoin Core (BIP61)

This feature has been disabled by default since Smartiecoin Core version 19.0.0.
Nodes on the network can not generally be trusted to send valid ("reject")
messages, so this should only ever be used when connected to a trusted node.

Since Smartiecoin Core version 20.0.0 there are extra changes:
...
- It is possible to run Smartiecoin Core as an I2P service and connect to such
...
- The `fundrawtransaction`, `sendmany`, `sendtoaddress`, and
  ...
  fee estimation methods, `SMT/kB` and `duff/B`. The target is the fee
...
- `testmempoolaccept` and `sendrawtransaction` no longer return the P2P
  ...
  Section [_Removal of reject network messages from Smartiecoin Core (BIP61)_](#removal-of-reject-network-messages-from-smartiecoin-core-bip61) above for
...
### Smartiecoin-specific changes in existing RPCs:
...
- The `smartiecoin-cli -getinfo` command now displays the wallet name and balance for
...
Please check `Help -> Command-line options` in Qt wallet or `smartiecoind --help` for
more information.
...
python test framework and `smartiecoin-cli` tool both take advantage of this, so
for example:

```sh
smartiecoin-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
smartiecoin-cli -named createwallet mywallet load_on_startup=1
```
...
with Smartiecoin’s product needs, such as SegWit and RBF, are excluded from our
...
[i2p]: https://github.com/SmartiesCoin/Smartiecoin/blob/v20.x/doc/i2p.md
[guix-readme]: https://github.com/SmartiesCoin/Smartiecoin/blob/v20.x/contrib/guix/README.md
[json-rpc]: https://github.com/SmartiesCoin/Smartiecoin/blob/v20.x/doc/JSON-RPC-interface.md#parameter-passing
[set-of-changes]: https://github.com/SmartiesCoin/Smartiecoin/compare/v19.3.0...dashpay:v20.0.0
