# Smartiecoin Core version v20.1.0
...
In this release, we are taking a significant step towards enhancing the Smartiecoin wallet's usability by enabling Hierarchical
Deterministic (HD) wallets by default. This change aligns the behavior of `smartiecoind` and `smartiecoin-qt` with the previously
optional `-usehd=1` flag, making HD wallets the standard for all users.

While HD wallets are now enabled by default to improve user experience, Smartiecoin Core still allows for the creation of
...
Modified the Dockerfile to exclude the `smartiecoin-qt` graphical wallet interface from the set of binaries copied to
`/usr/local/bin` during the Docker image build process. This change streamlines the Docker image, making it more
suitable for server environments and headless applications, where the graphical user interface of `smartiecoin-qt` is not
...
the v18.1 release.
- The `startupnotify` option is used to specify a command to execute when Smartiecoin Core has finished with its startup
sequence.
...
- New Discreet mode for hiding balances in Smartiecoin-Qt
- Added a "Close all wallet" menu item for Smartiecoin-Qt
...
[set-of-changes]: https://github.com/SmartiesCoin/Smartiecoin/compare/v20.0.4...dashpay:v20.1.0
