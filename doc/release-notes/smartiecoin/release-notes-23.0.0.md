# Smartiecoin Core version v23.0.0
...
The compact block filter index format has been updated to include Smartiecoin special transaction data, providing feature parity with bloom filters for SPV client support. This change is incompatible with existing blockfilter indexes. Existing blockfilter indexes will automatically be re-created with the new version.

- The blockfilter index now includes fields from Smartiecoin special transactions:
...
- SPV clients can now detect and track Smartiecoin-specific transactions
...
* Performance of block validation in Smartiecoin Core has been significantly improved, index from
  scratch and reindex are up to 20% faster compared to v22.x.
* To help prevent fingerprinting transactions created by the Smartiecoin Core wallet, change output
  amounts are now randomized. (#6685)
* Smartiecoin Core will no longer migrate EvoDb databases generated in v19 and v20, users upgrading
...
- `PROTO_VERSION` has been bumped to `70238` with the introduction of the `platformban` p2p message.
  This message allows evonodes to initiate PoSe masternode banning in Smartiecoin Core for evonodes which are
  not providing adequate service on Smartiecoin Platform.
- `cycleHash` field in `isdlock` message will now represent a DKG cycle starting block of the signing quorum instead of a DKG cycle starting block corresponding to the current chain height. While this is fully backwards compatible with older versions of Smartiecoin Core, other implementations might not be expecting this, so the P2P protocol version was bumped to 70237. (#6608)
...
* The RPCs `protx register_legacy`, `protx register_fund_legacy`, `protx register_prepare_legacy` and
  `protx update_registrar_legacy` have been deprecated in Smartiecoin Core v23 and may be removed in a future version.
  They can be re-enabled with the runtime argument `-deprecatedrpc=legacy_mn`.

* The argument `legacy` in `bls generate` has been deprecated in Smartiecoin Core v23 and may be ignored in a future version.
...
* The `instantsendtoaddress` RPC was deprecated in Smartiecoin Core v0.15 and is now removed. (#6686)
...
* The RPCs `masternode current` and `masternode winner` were deprecated in Smartiecoin Core v0.17 and are now removed. The `getpoolinfo` RPC was deprecated in Smartiecoin Core v0.15 and is now removed. (#6567)
...
Smartiecoin Core v23 introduces support for extended masternode addresses, replacing legacy single-endpoint fields and
...
* If the masternode is eligible for extended addresses, `protx register{,_evo}` and `protx register_fund{,_evo}` will:
  ...
  * Continue to allow specifying only the port number for `platformP2PAddrs` and `platformHTTPSAddrs`, pairing it with the address
    ...
    * This method of entry may not be available in future releases of Smartiecoin Core and operators are recommended to switch over to
...
GCC 11.1 or later, or Clang 16.0 or later, are now required to compile Smartiecoin Core. (#6389)
...
- Fixed an issue where CoinJoin funds mixed in the Smartiecoin Android wallet were invisible when importing the mnemonic into Smartiecoin Core. Descriptor Wallets now include an additional default descriptor for mobile CoinJoin funds, ensuring seamless wallet migration and complete fund visibility across different Smartiecoin wallet implementations.
...
[set-of-changes]: https://github.com/SmartiesCoin/Smartiecoin/compare/v22.1.3...dashpay:v23.0.0