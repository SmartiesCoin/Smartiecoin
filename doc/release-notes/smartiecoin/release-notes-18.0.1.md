Smartiecoin Core version v18.0.1
=========================
...
been added to support Smartiecoin Platform. This update introduces versioning to
...
Governance
----------
Several improvements have been made to Smartiecoin’s DAO governance system.
The governance proposal fee has been reduced from 5 Smartiecoin to 1 Smartiecoin following
...
`salvage` command in the `smartiecoin-wallet` tool.
...
- The `testnet` field in `smartiecoin-cli -getinfo` has been renamed to `chain` and
...
deprecated. `size` will only be returned if `smartiecoind` is started with
`-deprecatedrpc=size`.
...
---------------------------
This release introduces over 1000 updates from Bitcoin v0.18/v0.19/v0.20 as well as numerous updates from Bitcoin v0.21 and more recent versions. This includes multi-wallet support in the GUI, support for partially signed transactions (PSBT), Tor version 3 support, and a number of other updates that will benefit Smartiecoin users. Bitcoin changes that do not align with Smartiecoin’s product needs, such as SegWit and RBF, are excluded from our backporting. For additional detail on what’s included in Bitcoin, please refer to their release notes – v0.18, v0.19, v0.20.
...
See detailed [set of changes](https://github.com/SmartiesCoin/Smartiecoin/compare/v0.17.0.3...dashpay:v18.0.1).
