...
Running smartiecoind as a daemon does not require any manual configuration. You may
set the `rpcauth` setting in the `smartiecoin.conf` configuration file to override
...
`/etc/smartiecoin/smartiecoin.conf` with the current systemd, OpenRC, and Upstart init
files out-of-the-box. This is because the command line options specified in the
init files take precedence over the configurations in
`/etc/smartiecoin/smartiecoin.conf`. However, some init systems have their own
