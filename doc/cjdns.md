# CJDNS support in Smartiecoin Core

It is possible to run Smartiecoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Smartiecoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Smartiecoin network and individual nodes.
...
before it will work with your Smartiecoin Core node. This is described in steps
...
## Run Smartiecoin Core with CJDNS

Once you are connected to the CJDNS network, the following Smartiecoin Core
...
When enabled, this option tells Smartiecoin Core that it is running in an
...
IPv6 local network. This helps Smartiecoin Core perform better address management:
...
CJDNS support was added to Smartiecoin Core in version 21.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `smartiecoin-cli -addrinfo` to see the
...
## CJDNS-related information in Smartiecoin Core

There are several ways to see your CJDNS address in Smartiecoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `smartiecoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `smartiecoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `smartiecoin-cli help getnodeaddresses` for details.
