...
CONFIG_HEADER = $(top_builddir)/src/config/smartiecoin-config.h
CONFIG_CLEAN_FILES = libsmartiecoinconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libsmartiecoinconsensus.pc.in \
...
OTOOL64 = 
PACKAGE = smartiecoin
PACKAGE_BUGREPORT = https://github.com/vishcoin/smartiecoin/issues
PACKAGE_NAME = Smartiecoin Core
PACKAGE_STRING = Smartiecoin Core 1.2.0
...
pkgconfig_DATA = libsmartiecoinconsensus.pc
BITCOIND_BIN = $(top_builddir)/src/smartiecoind$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/smartiecoin-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/smartiecoin-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = Smartiecoin-Qt.app
OSX_DMG = Smartiecoin-Core.dmg
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_smartiecoin_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_smartiecoin_coverage.info test_smartiecoin.info
...
src/config/smartiecoin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/smartiecoin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/smartiecoin-config.h
$(top_srcdir)/src/config/smartiecoin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/smartiecoin-config.h src/config/stamp-h1
libsmartiecoinconsensus.pc: $(top_builddir)/config.status $(srcdir)/libsmartiecoinconsensus.pc.in
...
$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "Smartiecoin-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
#test_smartiecoin.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_smartiecoin -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_smartiecoin_filtered.info: test_smartiecoin.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_smartiecoin_filtered.info
#	$(MKDIR_P) qa/tmp
...
#rpc_test.info: test_smartiecoin_filtered.info
...
#test_smartiecoin_coverage.info: baseline_filtered_combined.info test_smartiecoin_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_smartiecoin_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_smartiecoin_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_smartiecoin_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_smartiecoin.coverage/.dirstamp:  test_smartiecoin_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_smartiecoin.coverage/.dirstamp total.coverage/.dirstamp

# Smartiecoin: disable the test until upstream switches from convoluted comparison tool
...
clean-local:
	rm -rf coverage_percent.txt test_smartiecoin.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
