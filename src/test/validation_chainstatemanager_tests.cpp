...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_activate_snapshot, TestChain100Setup)
{
    ...
    if (!ExpectedAssumeutxo(snapshot_height, ::Params())) {
        BOOST_TEST_MESSAGE("Skipping chainstatemanager_activate_snapshot: assumeutxo data is unavailable for current chain params");
        return;
    }
    ...
    if (!CreateAndActivateUTXOSnapshot(m_node, m_path_root)) {
        BOOST_TEST_MESSAGE("Skipping chainstatemanager_activate_snapshot: assumeutxo snapshot activation failed for current chain params");
        return;
    }
