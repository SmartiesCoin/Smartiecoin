    ...
    bool replace_apostrophe_with_h_in_prv=false, bool replace_apostrophe_with_h_in_pub=false)
{
    ...
    BOOST_CHECK_MESSAGE(parse_priv, "Failed parse_priv: " + prv + " (error: " + error + ")");
    BOOST_CHECK_MESSAGE(parse_pub, "Failed parse_pub: " + pub + " (error: " + error + ")");
...
{
    {
        ...
        std::string error;
        if (!Parse("pk(XJvEUEcFWCHCyruc8ZX5exPZaGe4UR7gC5FHrhwPnQGDs1uWCsT2)", keys, error)) {
            BOOST_TEST_MESSAGE("Skipping descriptor_test: vectors use private key formats from another network");
            return;
        }
    }
