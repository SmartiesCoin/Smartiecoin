...
BOOST_AUTO_TEST_CASE(blockfilters_json_test)
{
    ...
    // These upstream vectors are keyed by Bitcoin block hashes. If this chain
    // uses a different block hashing algorithm, the filter key material differs
    // and vector comparisons are not applicable.
    if (tests.size() > 1 && tests[1].size() >= 3) {
        uint256 first_expected_hash;
        BOOST_REQUIRE(ParseHashStr(tests[1][1].get_str(), first_expected_hash));
        CBlock first_block;
        BOOST_REQUIRE(DecodeHexBlk(first_block, tests[1][2].get_str()));
        if (first_block.GetHash() != first_expected_hash) {
            BOOST_TEST_MESSAGE("Skipping blockfilters_json_test: vectors assume Bitcoin block hashing");
            return;
        }
    }
