...
BOOST_AUTO_TEST_CASE(packet_test_vectors) {
    ...
    static constexpr std::array<unsigned char, 4> BITCOIN_MAINNET_MESSAGE_START{
        0xf9, 0xbe, 0xb4, 0xd9
    };
    if (!std::equal(std::begin(Params().MessageStart()), std::end(Params().MessageStart()),
                    BITCOIN_MAINNET_MESSAGE_START.begin(), BITCOIN_MAINNET_MESSAGE_START.end())) {
        BOOST_TEST_MESSAGE("Skipping BIP324 packet test vectors: vectors are tied to Bitcoin mainnet message start");
        return;
    }
