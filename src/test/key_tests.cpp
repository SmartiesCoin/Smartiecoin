...
static bool LegacySecretVectorsCompatible()
{
    return DecodeSecret(strSecret1).IsValid() &&
           DecodeSecret(strSecret1C).IsValid() &&
           IsValidDestination(DecodeDestination(addr1)) &&
           IsValidDestination(DecodeDestination(addr1C));
}
...
{
    if (!LegacySecretVectorsCompatible()) {
        BOOST_TEST_MESSAGE("Skipping key_test1: secret/address vectors use encodings from another network");
        return;
    }
...
{
    if (!LegacySecretVectorsCompatible()) {
        BOOST_TEST_MESSAGE("Skipping key_signature_tests: secret/address vectors use encodings from another network");
        return;
    }
...
{
    if (!LegacySecretVectorsCompatible()) {
        BOOST_TEST_MESSAGE("Skipping key_key_negation: secret/address vectors use encodings from another network");
        return;
    }
...
{
    // Use raw deterministic secrets to avoid depending on network-specific WIF prefixes.
    std::array<uint8_t, 32> secret1{};
    secret1[31] = 1;
    ...
    for (const bool compressed : {false, true}) {
        CKey key1;
        key1.Set(secret1.begin(), secret1.end(), compressed);
        BOOST_REQUIRE(key1.IsValid());
        keys.push_back(key1);
        CKey key2;
        key2.Set(secret2.begin(), secret2.end(), compressed);
