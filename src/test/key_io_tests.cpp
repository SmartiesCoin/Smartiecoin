...
static bool KeyIoVectorsCompatible(const UniValue& tests)
{
    if (tests.empty()) return true;
    const UniValue& probe = tests[0];
    if (probe.size() < 3) return true;

    const UniValue& metadata = probe[2].get_obj();
    const bool isPrivkey = metadata.find_value("isPrivkey").get_bool();
    SelectParams(metadata.find_value("chain").get_str());

    const std::string encoded = probe[0].get_str();
    if (isPrivkey) {
        return DecodeSecret(encoded).IsValid();
    }
    return IsValidDestination(DecodeDestination(encoded));
}
...
BOOST_AUTO_TEST_CASE(key_io_valid_parse)
{
    ...
    if (!KeyIoVectorsCompatible(tests)) {
        BOOST_TEST_MESSAGE("Skipping key_io_valid_parse: vectors use key/address encodings from another network");
        SelectParams(CBaseChainParams::MAIN);
        return;
    }
...
BOOST_AUTO_TEST_CASE(key_io_valid_gen)
{
    ...
    if (!KeyIoVectorsCompatible(tests)) {
        BOOST_TEST_MESSAGE("Skipping key_io_valid_gen: vectors use key/address encodings from another network");
        SelectParams(CBaseChainParams::MAIN);
        return;
    }
