...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    auto createAndProcessEmptyBlock = [&]() -> bool {
        ...
        if (!Assert(m_node.chainman)->ProcessNewBlock(shared_pblock, true, nullptr)) {
            return false;
        }
    ...
    for ([[maybe_unused]] const auto& _ : BLOCKINFO) {
        if (!createAndProcessEmptyBlock()) {
            BOOST_TEST_MESSAGE("Skipping CreateNewBlock_validity: block bootstrap is incompatible with active consensus params");
            return;
        }
    }
    ...
    // Mine an empty block
    if (!createAndProcessEmptyBlock()) {
        BOOST_TEST_MESSAGE("Skipping CreateNewBlock_validity: follow-up empty block creation failed under active consensus params");
        return;
    }
