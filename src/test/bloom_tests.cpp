...
{
    // Use a deterministic raw key so this test is independent from chain-specific WIF prefixes.
    std::vector<unsigned char> raw_secret(32, 0x00);
    raw_secret.back() = 0x01;
    CKey key;
    key.Set(raw_secret.begin(), raw_secret.end(), /*fCompressedIn=*/false);
