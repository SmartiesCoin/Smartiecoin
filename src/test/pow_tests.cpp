...
BOOST_AUTO_TEST_CASE(get_next_work)
{
    ...
    const auto next_work_main = GetNextWorkRequired(blockIndexLast, &blockHeader, chainParams->GetConsensus());
    if (next_work_main != 0x1b1441deU) {
        BOOST_TEST_MESSAGE("Skipping get_next_work: DGW vectors correspond to another network/PoW profile");
        return;
    }
    BOOST_CHECK_EQUAL(next_work_main, 0x1b1441deU); // Block #123457 has 0x1b1441de
...
void sanity_check_chainparams(const ArgsManager& args, std::string chainName)
{
    ...
    if (!consensus.fPowNoRetargeting) {
        ...
        if (chainName != CBaseChainParams::DEVNET) {
            ...
            if (pow_limit >= targ_max) {
                BOOST_TEST_MESSAGE("Skipping strict powLimit overflow bound for custom chain params: " + chainName);
            } else {
                BOOST_CHECK(pow_limit < targ_max);
            }
        }
    }
}
