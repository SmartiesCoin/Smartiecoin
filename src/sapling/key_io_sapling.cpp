...
namespace {
std::vector<uint8_t> ToUCharVector(const CDataStream& ss)
{
...
public:
    ...
    std::string operator()(const libzcash::SaplingPaymentAddress& zaddr) const
    {
        ...
        ss << zaddr;
        // ConvertBits requires unsigned char, while CDataStream stores std::byte.
        std::vector<unsigned char> seraddr = ToUCharVector(ss);
...
public:
    ...
    std::string operator()(const libzcash::SaplingExtendedFullViewingKey& extfvk) const
    {
        ...
        ss << extfvk;
        // ConvertBits requires unsigned char, while CDataStream stores std::byte.
        std::vector<unsigned char> serkey = ToUCharVector(ss);
        std::vector<unsigned char> data;
        // See calculation comment below
...
namespace KeyIO {
    ...
    libzcash::PaymentAddress DecodePaymentAddress(const std::string& str)
    {
        ...
        if (bech.encoding == bech32::Encoding::BECH32 &&
            bech.hrp == Params().Bech32HRP(CChainParams::SAPLING_PAYMENT_ADDRESS) &&
    ...
    {
        auto bech = bech32::Decode(str, /*limit_length=*/false);
        if (bech.encoding == bech32::Encoding::BECH32 &&
            bech.hrp == Params().Bech32HRP(CChainParams::SAPLING_EXTENDED_FVK) &&
    ...
    libzcash::SpendingKey DecodeSpendingKey(const std::string& str)
    {
        ...
        auto bech = bech32::Decode(str, /*limit_length=*/false);
        if (bech.encoding == bech32::Encoding::BECH32 &&
            bech.hrp == Params().Bech32HRP(CChainParams::SAPLING_EXTENDED_SPEND_KEY) &&
