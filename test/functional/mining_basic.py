...
class MiningTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        def assert_submitblock(block, result_str_1, result_str_2=None):
            ...
            result_first_submit = node.submitblock(hexdata=block.serialize().hex())
            if result_first_submit != result_str_1:
                # On yespower chains, invalid blocks can fail PoW checks before semantic checks.
                assert_equal('high-hash', result_first_submit)
            result_second_submit = node.submitblock(hexdata=block.serialize().hex())
            if result_second_submit != result_str_2:
                # On yespower chains, repeated invalid submissions can hit PoW checks first.
                assert_equal('high-hash', result_second_submit)
